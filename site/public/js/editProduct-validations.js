window.addEventListener("load", function () {
  ...
  let validateDiscount = function () {
    ...
    if (validator.isEmpty(discount.value, { ignore_whitespace: true })) {
      feedback =
        "Debe completar el descuento del producto con valores numéricos ";
  ...
  let validateCategories = function () {
    let someoneChecked = Array.prototype.filter.call(
      categories,
      (cat) => cat.checked
    );
    handleFeedback(
      categoriesWrapper,
      someoneChecked.length == 0 ? "El campo no puede estar vacío" : "",
      "categories"
    );
  };
  ...
  let validateImages = function (event) {
    ...
    handleFeedback(
      images,
      regex.test(event.target.value) ? "" : "Debe ser jpg, gif, png o jpeg"
    );
  };
